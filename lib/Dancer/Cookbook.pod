...
=head2 Using Moose with Moose mode

If you're using L<Moose>, you've probably noticed there's a small conflict in
keywords: Both Moose and Dancer provide C<before> and C<after> method
modifiers. To get around that, you can ask Dancer not to import those keywords.

    use Dancer ':moose';

This imports Dancer in Moose mode (implies Syntax mode), which does not import
C<before> nor C<after>.
