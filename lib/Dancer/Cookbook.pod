...
It's possible to return custom error, using the C<send_error> function. When you don't use a serializer, the C<send_error> function will take a string as first parameter (the message), and an optional HTTP code. While using a serializer, message can be a string, an arrayref or a hashref:

    get '/hello/:name' => sub {
        if (...) {
           halt send_error("you can't do that");
           # or
           halt send_error({reason => 'access denied', message => "no"});
        }
    };
