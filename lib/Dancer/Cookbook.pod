...
That script - C<mywebapp/mywebapp.pl> in the example above - can be used as a
standalone server or as a PSGI startup script (this will only be of interest if
you're running your web app via Plack/PSGI - more on that later).
