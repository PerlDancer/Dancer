...
Acrocnym for B<P>ull B<R>equest

=item * Contributor

A GitHub user who had forked and clone the official Dancer's repo, and
who has sent a PR.
...
the iontegration branch. If not, this is enough to refuse the PR (it
...
To avoid that, we're going to pull the commits of $user into a
...
First, we make sure we are sync with C<origin/devel>
...
have to go into C<temp> and inspect the history (with C<git log>).
