...
Accessor to the singleton that stores all the settings.
...
Public accessor for a setting. Allows to set or get a setting:
    ...
    setting foo => 42; # sets the setting "foo" to 42.

    my $value = setting 'foo'; # 42
...
Initializes the configuration registry of the application by loading the main
configuration file (C<config.yml>) then the environment configuration file
(C<environments/$env.yml>).
...
sub load {
    ...
    return 1 unless -f _conffile();

    # load YAML
    confess "Configuration file found but YAML is not installed"
      unless Dancer::ModuleLoader->load('YAML');

    _load_settings_from_yaml(_conffile());

    my $env = _environment_file();
    _load_settings_from_yaml($env) if -f $env;
...
sub _set_setting {
    ...
    $value = Dancer::Config->_normalize_setting($setting, $value);
