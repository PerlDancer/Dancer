...
one application per directory for example, you can do like the following.

This example uses the FastCGI dispatcher that comes with Dancer, but you can
imagine any other way of deployement describe in this guide. The only purpose
of this example is to show how to deploy multiple applications under the same
base directory/virtualhost.
...
RewriteRules in a .htaccess file directly withtin the application directories,
