...
as a simple cgi-script out-of-the-box. Under Apache or any web server supporting
.htaccess files and mod_rewrite. The Perl module Plack::Runner is required.

=head3 Running as a cgi-script using only .htaccess

Place this code in a file called .htaccess in your application's root folder:

    # BEGIN DANCER APPLICATION HTACCESS
    
    DirectoryIndex public/dispatch.cgi/
...
Please note that you may need to modify your .htaccess file to support additional
file types under the "Handle Public Files" section.

=head3 Running as a cgi-script under an Apache VirtualHost
