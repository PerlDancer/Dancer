...
a file called .htaccess (or directly in your VirtualHost configuration) in 
your application's root folder:
...
If you're going to host your application under the root path (B</>), add the
following directive:
