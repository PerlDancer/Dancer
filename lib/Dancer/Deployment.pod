...
You can use Plack::Builder to mount multiple Dancer applications on a L<PSGI>
webserver like L<Starman>. Please use the C<routes_per_app> option (should be 1
or true) in your config for preventing collisions of routes between Dancer
applications.
...
Start by creating a simple app.psgi file:
    ...
    # Now routes of $app1 will not conflict with routes of $app2
    setting routes_per_app  => true;
...
If you use multiple Dancer apps under Apache please setup in your config.yml a
following option:

    routes_per_app: 1

This will prevent collisions of routes between Dancer applications.
