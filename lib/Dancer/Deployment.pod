...
You will need plackup to start a socket :
    cd YOUR_PROJECT_PATH
    sudo -u www plackup -E production -s Starman --workers=2 -l THE_PASS_OF_YOUR_PLACKUP_SOCKET_HERE.sock -a bin/app.pl

The better wait to start this is to use 'daemontools' and place this lines with all environments variables in the "run" file.
