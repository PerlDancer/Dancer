...
=head3 Building on devel branch

Dancer switched to Dist::Zilla as a contribution builder. If you are developing
Dancer, you might now need Dist::Zilla to build and test Dancer:
...
See L<dzil.org|http://dzil.org> for more information

=head3 POD enhancements

Dancer's Dist::Zilla configuration allows us to use POD enhancements which
make the code more concise and less error-prone.

=over 4

=item * name and version

are created from the abstract which has this format:

	# ABSTRACT: tag line

=item * copyright and license information

are created automatically at the end of the file

=item * new directives

List of functions, methods and attributes are now created from corresponding
directives:
...
For more information pod enhancements, see L<dzil.org|http://dzil.org>.
