...
You don't have to start by hacking the code, spreading the word is valuable as
well. If you have a blog, just feel free to speak about Dancer.
...
If you find a typo or an error in the documentation you can submit a patch with
your fix (see "Patch submission").

=head2 Quality Assurance

We call "Quality Assurance" the fact that we keep in mind that Dancer should be
able to install for all Perl versions since 5.8, on any platform.

We should avoid regressions as much as possible and preserve backward
compatibility should be always in mind when refactoring.
...
In order to make the developing Dancer easier, git-flow is used to handle
pull-requests, that means contributors must work on their C<devel> branch
rather than on their C<master>.
...
At least, you should notify the core team on IRC, on C<irc.perl.org>,
chan C<#dancer>.
