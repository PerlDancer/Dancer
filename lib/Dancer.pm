...
=method from_json ($structure, %options)
...
=method from_xml ($structure, %options)
...
=cut
sub get {
    map { my $r = $_; Dancer::App->current->registry->universal_add($r, @_) } qw(head get)
}
...
B<Deprecated.> Use C<set logger => 'console'> to change current logger engine.

=cut
sub logger {
    Dancer::Deprecation->deprecated(reason => "use 'set logger'",
                                    fatal => 0,
                                    version=>'1.3050');
    set(logger => @_)
}
...
I<This method should be called from a route handler>.  It's an alias
for the L<Dancer::Request params
accessor|Dancer::Request/"params">. It returns an hash reference to
all defined parameters. Check C<param> bellow to access quickly to a
...
=cut
sub send_error {
    Dancer::Error->new(message => $_[0], code => $_[1] || 500)->render()
}
...
=method to_json ($structure, %options)
...
=method to_xml ($structure, %options)
...
sub _init_script_dir {
    ...
    my ($res, $error) =
      Dancer::ModuleLoader->use_lib(Dancer::FileUtils::path_no_verify($appdir, 'lib'));
