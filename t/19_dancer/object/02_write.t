...
# write methods tests.
ok( $script->write_bg(catfile($dancer_app_dir, 'public', 'images', 'perldancer-bg.jpg')),
"write_data_to_file method sucessfully wrote a file.");

ok( $script->write_file(catfile($dancer_app_dir,'t','test'),$template), "write_file successfully writes a file.");

$script = Dancer::Script->new(appname => 'Hello::World', path => $tmpdir, check_version => '1');
$dancer_app_dir = $script->{dancer_app_dir};

#tests for long name.

# TODO-to-every-t: Supress the print messages from the *print* outputs. 
ok( $script->safe_mkdir($dancer_app_dir),"safe_mkdir method sucessfully wrote a long name directory.");
